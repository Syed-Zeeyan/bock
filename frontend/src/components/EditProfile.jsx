...
const EditProfile = () => {
    ...
    const editProfileHandler = async () => {
        ...
        try {
            setLoading(true);
            // const res = await axios.post('http://localhost:8000/api/v1/user/profile/edit', formData, {
                const res = await axios.post(`${import.meta.env.VITE_BASE_URL}/user/profile/edit`, formData, {
    ...
    return (
        <div className='flex max-w-2xl mx-auto p-10 bg-black text-white'>
            <section className='flex flex-col gap-6 w-full'>
                ...
                <div className='flex items-center justify-between bg-[#1A1A1A] rounded-xl p-4'>
                    ...
                    <Button onClick={() => imageRef.current.click()} className='bg-purple-600 hover:bg-purple-700 h-8'>Change Photo</Button>
                </div>
                <div>
                    <h1 className='font-bold text-xl mb-2 text-gray-300'>Bio</h1>
                    <Textarea value={input.bio} onChange={(e) => setInput(prev => ({ ...prev, bio: e.target.value }))} className="bg-[#1A1A1A] text-gray-300 focus-visible:ring-transparent" />
                </div>
                <div>
                    <h1 className='font-bold mb-2 text-gray-300'>Gender</h1>
                    <Select defaultValue={input.gender} onValueChange={selectChangeHandler}>
                        <SelectTrigger className="w-full bg-[#1A1A1A] text-gray-300">
                            <SelectValue />
                        </SelectTrigger>
                        <SelectContent className="bg-[#1A1A1A] text-gray-300">
                            <SelectGroup>
                ...
                <div className='flex justify-end'>
                    <Button onClick={editProfileHandler} className='w-fit bg-purple-600 hover:bg-purple-700' disabled={loading}>
