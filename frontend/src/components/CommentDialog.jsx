...
const CommentDialog = ({ open, setOpen }) => {
    ...
    const sendMessageHandler = async () => {
        try {
            // const res = await axios.post(`http://localhost:8000/api/v1/post/${selectedPost._id}/comment`, { text }, {
                const res = await axios.post(`${import.meta.env.VITE_BASE_URL}/post/${selectedPost._id}/comment`, { text }, { 
    ...
    return (
        <Dialog open={open}>
            <DialogContent onInteractOutside={() => setOpen(false)} className="max-w-3xl p-0 flex flex-col bg-[#1A1A1A] border-[#333]">
                <div className='flex flex-1'>
                    <div className='w-1/2'>
                        <img
                            src={selectedPost?.image}
                    ...
                    <div className='w-1/2 flex flex-col justify-between'>
                        <div className='flex items-center justify-between p-4'>
                            ...
                            <Dialog>
                                ...
                                <DialogContent className="flex flex-col items-center text-sm text-center bg-[#1A1A1A] border-[#333] text-gray-300">
                        ...
                        <div className='p-4'>
                            <div className='flex items-center gap-2'>
                                <input
                                    ...
                                    className='w-full outline-none border text-sm border-[#333] p-2 rounded bg-[#333] text-gray-300'
                                />
                                <Button disabled={!text.trim()} onClick={sendMessageHandler} variant="outline" className='text-purple-500 border-purple-500 hover:bg-purple-500 hover:text-white'>Send</Button>
                            </div>
                        </div>
                    </div>
                </div>
